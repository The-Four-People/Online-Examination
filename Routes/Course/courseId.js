            ...
            ids.map((id) => {
                ...
                original_quiz.find((curr) => {
                    ...
                    if (curr.id === parseInt(id) + 1) {
                        ...
                        if (
                            curr.answer === Object.values(attempt_quiz[id])[0]
                        ) {
                            marks += curr.marks;
                            // console.log(marks);
                        }
...
router.post(
    ...
    async (req, res) => {
        try {
            const [original_quiz, original_quiz_length] =
                await getQuestionCounts(req.params.code, req.params.test);
...
async function getQuestionCounts(code, test) {
    ...
    let testDoc = await course
        .findOne({ test_name: test })
        .select('-_id')
        .exec();
    if (testDoc) {
