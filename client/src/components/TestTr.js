...
const RenderForm = ({ courseCode, testCode, getTest, test }) => {
    ...
    function handleFormSubmit(e) {
        ...
        // check answer in option
        let isAnswerInOption = quiz.options.some(
        ...
        if (!isAnswerInOption) {
            setMsg({
                ok: true,
                text: 'Answer is not in Options (Check spacing and case)',
            });
            setTimeout(() => setMsg({ ok: false, text: '' }), 5000);
            return;
        }

        // check duplicate id
        let isDuplicate = test.quiz.some(
            (ele) => ele.id.toString() === quiz.id
        );
        if (isDuplicate) {
            setMsg({
                ok: true,
                text: `Question with ID ${quiz.id} already exists`,
            });
            setTimeout(() => setMsg({ ok: false, text: '' }), 5000);
            return;
        }
