		...
		<>
			{isLoading ? (
				...
				>
					...
					{test.showMarks && <DisplayQuestions question={test.attempt} />}
				</div>
			) : (
				...
				<table className="table-info" style={{ height: "99%", width: "100%" }}>
					<tbody className="big-table-content">
						...
						<tr>
							...
							<td>{test.showMarks ? marks.marks : "Not Public yet"}</td>
						</tr>
						<tr>
							<th>Percentage</th>
							<td>
		...
		<>
			...
			{arrowClicked ? (
				<div className="questions">
					{question.map((ques) => {
						return <DisplayIndividualQuestion key={ques.id} question={ques} />;
					})}
				</div>
			) : null}
		</>
						...
						{question.options.map((option) => (
							<li key={option}>
								...
								{option === question.c_ans ? (
									...
									option === question.answer && <AiOutlineClose />
								)}
							</li>
						))}
					</ul>
